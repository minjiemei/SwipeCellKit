...
class SwipeActionsView: UIView {
    ...
    var safeAreaMargin: CGFloat {
        ...
        guard let scrollView = safeAreaInsetView else { return 0 }
    ...
    func addButtons(for actions: [SwipeAction], withMaximum size: CGSize, contentEdgeInsets: UIEdgeInsets) -> [SwipeActionButton] {
        ...
        buttons.enumerated().forEach { index, button in
            ...
            let wrapperView = SwipeActionButtonWrapperView(frame: frame, action: action, orientation: orientation, contentWidth: minimumButtonWidth, rightSpacing: options.rightSpacing)
            ...
            let topConstraint = wrapperView.topAnchor.constraint(equalTo: topAnchor, constant: options.topSpacing ?? contentEdgeInsets.top)
            topConstraint.priority = contentEdgeInsets.top == 0 ? .required : .defaultHigh
            topConstraint.isActive = true

            let bottomConstraint = wrapperView.bottomAnchor.constraint(equalTo: bottomAnchor, constant: -1 * (options.topSpacing ?? contentEdgeInsets.bottom))
...
class SwipeActionButtonWrapperView: UIView {
    ...
    init(frame: CGRect, action: SwipeAction, orientation: SwipeActionsOrientation, contentWidth: CGFloat, rightSpacing: CGFloat? = 0) {
        switch orientation {
        case .left:
            contentRect = CGRect(x: frame.width - (contentWidth - (rightSpacing ?? 0)), y: 0, width: contentWidth - (rightSpacing ?? 0), height: frame.height)
        case .right:
            contentRect = CGRect(x: 0, y: 0, width: contentWidth - (rightSpacing ?? 0), height: frame.height)
        }
    ...
    override func draw(_ rect: CGRect) {
        ...
        if let actionBackgroundColor = actionBackgroundColor, let context = UIGraphicsGetCurrentContext() {
